...
<div class="container mx-auto max-w-6xl px-4 py-8">
	<!-- Header -->
	<div class="mb-8">
		<h1 class="mb-2 text-4xl font-bold">📊 Analytics</h1>
	...
	<div class="mb-8 grid grid-cols-1 gap-4 md:grid-cols-3">
		<div class="rounded-lg border border-gray-200 bg-white p-6 shadow-md">
			<div class="flex items-center justify-between">
				<div>
					<p class="mb-1 text-sm text-gray-600">Vues totales</p>
		...
		<div class="rounded-lg border border-gray-200 bg-white p-6 shadow-md">
			<div class="flex items-center justify-between">
				<div>
					<p class="mb-1 text-sm text-gray-600">Clics totaux</p>
		...
		<div class="rounded-lg border border-gray-200 bg-white p-6 shadow-md">
			<div class="flex items-center justify-between">
				<div>
					<p class="mb-1 text-sm text-gray-600">Taux de clic (CTR)</p>
	...
	<div class="mb-8 rounded-lg border border-gray-200 bg-white p-6 shadow-md">
		<h2 class="mb-4 text-xl font-bold">📅 Vues des 7 derniers jours</h2>
		<div id="chart-container" class="relative">
			{#if chartWidth > 0}
				<svg width={chartWidth} height={chartHeight} class="w-full">
					...
					{#each stats.viewsByDay as day, i}
						...
						<!-- Bar -->
						<rect {x} {y} width={barWidth} height={barHeight} rx="4" fill="url(#barGradient)" />
			...
			{:else}
				<div class="py-8 text-center text-gray-500">Chargement du graphique...</div>
			{/if}
		</div>
	</div>
	...
	<!-- Top Blocks Table -->
	<div class="mb-8 rounded-lg border border-gray-200 bg-white p-6 shadow-md">
		<h2 class="mb-4 text-xl font-bold">🏆 Blocs les plus cliqués</h2>
		{#if stats.blocksByClicks.length > 0}
			<div class="overflow-x-auto">
				<table class="w-full">
					<thead>
						<tr class="border-b border-gray-200">
							<th class="px-4 py-3 text-left text-sm font-semibold text-gray-600">Bloc</th>
							<th class="px-4 py-3 text-left text-sm font-semibold text-gray-600">Type</th>
							<th class="px-4 py-3 text-right text-sm font-semibold text-gray-600">Clics</th>
						</tr>
					</thead>
					<tbody>
						{#each stats.blocksByClicks as block}
							<tr class="border-b border-gray-100 hover:bg-gray-50">
								...
								<td class="px-4 py-3 text-right font-bold">{block.clicks.toLocaleString()}</td>
							</tr>
						{/each}
					</tbody>
				</table>
		...
		{:else}
			<div class="py-8 text-center text-gray-500">
				...
				<p class="mt-2 text-sm">
					Publiez votre page et partagez-la pour commencer à collecter des données.
				</p>
			</div>
		{/if}
	</div>
	...
	{#if pages.length > 0}
		<div class="rounded-lg border border-gray-200 bg-white p-6 shadow-md">
			<h2 class="mb-4 text-xl font-bold">📄 Vos pages</h2>
			<div class="space-y-2">
				{#each pages as page}

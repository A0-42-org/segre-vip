...
<div
	class="social-block flex justify-center gap-3"
...
>
	{#each content.socials as social (social.platform)}
		<a
			...
			style="width: 48px; height: 48px; border-radius: 50%; transition: all 0.2s ease; text-decoration: none;"
			title={social.label || social.platform}
			onclick={() => onClick?.(social)}
		>
			<span class="social-icon text-xl">{iconMap[social.platform.toLowerCase()] || '🔗'}</span>
		</a>
	{/each}
</div>
